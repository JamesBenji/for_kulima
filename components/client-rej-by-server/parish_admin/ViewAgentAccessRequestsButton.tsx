...
import GrantAgentAccessButton from "./GrantAgentAccessButton";
import toast from "react-hot-toast";
import Image from "next/image";

const supabase = createClient();

export default function ViewAgentAccessRequestsButton() {
  ...
  const makeAPIcall = async () => {
    setIsLoading(true);
    try {
      const requests = await fetch("/api/view-agent-requests", {
        method: "GET",
      });
      ...
      <div>
        {requests && requests?.length > 0 ? (
          <h1 className="font-semibold tracking-wide text-lg mb-4">
            Field agent account access requests ({requests?.length})
          </h1>
        ) : (
