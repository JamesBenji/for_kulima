...
import {
  ...
  isParishAdmin,
} from "@/lib/parish_admin_funcs/functions";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const mayBeDistrictAdminEmail = (await supabase.auth.getUser()).data.user
      ?.email;
    ...
    if (mayBeDistrictAdminEmail) {
      ...
      if (target_mail) {
        const result = await approveAgentAccessRequest(target_mail, supabase);
